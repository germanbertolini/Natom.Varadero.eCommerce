...
namespace natom.ecomm.sync.routine.rubro
{
    class Program
    {
        private static bool _lanzadoManualmente = false;
        private static string _ejecucionId = "";
        private static string _endPointRelativeAddress = "/SyncRubro/Post";
        private static string _endPointRelativeAddressGetScript = "/SyncRubro/GetScriptSQL";

        static void Main(string[] args)
        {
            Console.WriteLine("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
            Console.WriteLine("%%                   RubroRoutine                %%");
            Console.WriteLine("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
            Console.WriteLine("                                                   ");
            ...
            if (args.Length == 0)
            {
                ...
                LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "EJECUCIÓN [" + _ejecucionId + "] MANUAL");
            }
            else
            {
                _ejecucionId = args[0];
            ...
            LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "INICIO RUTINA RubroRoutine");

            try
            {
                Console.WriteLine("> Obteniendo Script SQL del servidor...");
                LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "OBTENIENDO SCRIPT SQL DEL SERVIDOR");
                ...
                LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "EJECUTANDO SENTENCIA SQL PARA OBTENER DATOS...");

                string query = taskPost.Result.Data;
                List<Rubro> dataToSync = new List<Rubro>();
                using (var db = new DbVaraderoContext())
                {
                    dataToSync = db.Database.SqlQuery<Rubro>(query).ToList();
                }

                Console.WriteLine("> Sincronizando datos al servidor eCommerce...");
                LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "SINCRONIZANDO DATOS AL SERVIDOR ECOMMERCE...");
                ...
                if (task.IsCompleted && task.Result.Success)
                {
                    ...
                    LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "FIN RUTINA RubroRoutine EXITOSA");
                }
                else
                {
                    Console.WriteLine("> Registrando fin de operacion con errores en el LOG...");
                    LogManager.LogInfo("routine.rubro", _ejecucionId, "Program.Main", "FIN RUTINA RubroRoutine CON ERRORES DEL LADO DEL SERVIDOR", new { taskResult = task.Result });
                }
            }
            catch (Exception ex)
            {
                LogManager.LogException("routine.rubro", _ejecucionId, "Program.Main", ex);
