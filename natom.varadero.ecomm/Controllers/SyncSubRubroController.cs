...
{
    public class SyncSubRubroController : BaseController
    {
        [HttpPost]
        public ActionResult Post(List<SubRubro> data)
        {
            var response = new EndpointResponse<string>();
            try
            {
                LogManager.Instance.LogInfo(null, "/SyncSubRubro/Post", "INICIO PERSISTENCIA DE DATOS", new ReceivedDataInfo().BuildInfo<SubRubro>(data));

                eCommStatusManager.Instance.RegisterStartedSync();
                using (var db = new DbEcommerceContext())
                {
                    db.SubRubros.RemoveRange(db.SubRubros);
                    db.SubRubros.AddRange(data);
                    db.SaveChanges();
                }
                response.Success = true;

                LogManager.Instance.LogInfo(null, "/SyncSubRubro/Post", "FIN PERSISTENCIA DE DATOS");
            }
            catch (Exception ex)
            {
                response.Success = false;
                response.ErrorMessage = (ex.InnerException ?? ex).Message;
                LogManager.Instance.LogException("/SyncSubRubro/Post", ex, Request);
            }
            finally
            {
