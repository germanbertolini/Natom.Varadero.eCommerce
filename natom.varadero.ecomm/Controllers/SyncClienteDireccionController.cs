...
{
    public class SyncClienteDireccionController : BaseController
    {
        [HttpPost]
        public ActionResult Post(List<ClienteDireccion> data)
        {
            var response = new EndpointResponse<string>();
            try
            {
                LogManager.Instance.LogInfo(null, "/SyncClienteDireccion/Post", "INICIO PERSISTENCIA DE DATOS", new ReceivedDataInfo().BuildInfo<ClienteDireccion>(data));
                ...
                using (var db = new DbEcommerceContext())
                {
                    ...
                    db.ClientesDirecciones.AddRange(data);
                    db.SaveChanges();
                }
                response.Success = true;

                LogManager.Instance.LogInfo(null, "/SyncClienteDireccion/Post", "FIN PERSISTENCIA DE DATOS");
            }
            catch (Exception ex)
            {
                response.Success = false;
                response.ErrorMessage = (ex.InnerException ?? ex).Message;
                LogManager.Instance.LogException("/SyncClienteDireccion/Post", ex, Request);
            }
            finally
            {
        ...
        {
            var response = new EndpointResponse<string>();
            string schedule = "ClienteDireccionRoutine";
