...
{
    public class SyncMarcaController : BaseController
    {
        [HttpPost]
        public ActionResult Post(List<Marca> data)
        {
            var response = new EndpointResponse<string>();
            try
            {
                LogManager.Instance.LogInfo(null, "/SyncMarca/Post", "INICIO PERSISTENCIA DE DATOS", new ReceivedDataInfo().BuildInfo<Marca>(data));

                eCommStatusManager.Instance.RegisterStartedSync();
                using (var db = new DbEcommerceContext())
                {
                    db.Marcas.RemoveRange(db.Marcas);
                    db.Marcas.AddRange(data);
                    db.SaveChanges();
                }
                response.Success = true;

                LogManager.Instance.LogInfo(null, "/SyncMarca/Post", "FIN PERSISTENCIA DE DATOS");
            }
            catch (Exception ex)
            {
                response.Success = false;
                response.ErrorMessage = (ex.InnerException ?? ex).Message;
                LogManager.Instance.LogException("/SyncMarca/Post", ex, Request);
            }
            finally
            {
